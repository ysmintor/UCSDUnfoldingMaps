...
public class EarthquakeCityMap extends PApplet {
	...
	/**
	 * Helper function to deal with Click on cities
	 */
	private void checkCitiesForClick() {
		// find which city is clicked
		for (Marker cityMarker : cityMarkers) {
			if (!cityMarker.isHidden() && cityMarker.isInside(map, mouseX, mouseY)) {
				...
				// find earthquakes which threat the select city
				for (Marker quake : quakeMarkers) {
					EarthquakeMarker earthquake = (EarthquakeMarker) quake;
					if (lastClicked.getDistanceTo(earthquake.getLocation()) > earthquake.threatCircle())
	...
	/**
	 * Helper function to deal with Click on Earthquakes
	 */
	private void checkEarthquakeForClick() {
		// find which earthquake is clicked
		for (Marker m : quakeMarkers) {
			...
			if (!quakeMarker.isHidden() && quakeMarker.isInside(map, mouseX, mouseY)) {
				...
				// find earthquakes which threat  cities
				for (Marker mHide : cityMarkers) {
					if (mHide.getDistanceTo(quakeMarker.getLocation()) > quakeMarker.threatCircle())
						mHide.setHidden(true);
				}
				return;
			}
